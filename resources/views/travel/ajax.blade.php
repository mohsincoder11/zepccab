<script>
    ...
    $(document).ready(function() {
        ...
        $("#submitadd").on('click', function() {
            ...
            $(".text-danger").remove();
            $(".messages").html("");
            $("#AddTravel").unbind('submit').bind('submit', function() {
                ...
                var postData = new FormData($("#AddTravel")[0]);
                $.ajax({
                    cache:false,
                    contentType: false,
                    processData: false,
...
// remove the error
                        ...
                        if(response.success == true) {
                            ...
                            toastr.success('Added Successfully.', 'Travel Type', {timeOut: 5000});
                            $("#AddTravel")[0].reset();
                ...
                $.ajax({
                    ...
                    success:function(response) {
                        if(response.success == true) {
                            ...
                            manageTable.ajax.reload(null, false);
                            // close the modal
                            toastr.success('Deleted Successfully.', 'Travel Type', {timeOut: 3000});
                            $("#modalConfirmDelete").modal('hide');
                            $(".loader-ajax").hide();

                        } else {
                            toastr.error('Not Deleted.', 'Rental', {timeOut: 3000});
                        }
                    }
                });
            }); // click remove btn
