<script>
    ...
    $(document).ready(function() {
        ...
        $("#submitadd").on('click', function() {
            ...
            $(".text-danger").remove();
            $(".messages").html("");
            $("#AddCompany").unbind('submit').bind('submit', function() {
                ...
                var postData = new FormData($("#AddCompany")[0]);
                $.ajax({
                    cache: false,
                    contentType: false,
                    processData: false,
                    ...
                    success: function(response) {
                        ...
                        if (response.success == true) {
                            toastr.success('Added Successfully.', 'Company', {
                                timeOut: 5000
                            });
                            $("#AddCompany")[0].reset();
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success == true) {
                            ...
                            manageTable.ajax.reload(null, false);
                            // close the modal
                            toastr.success('Deleted Successfully.', 'Company', {
                                timeOut: 3000
                            });
                            ...
                            toastr.error('Not Deleted.', 'Company', {
                                timeOut: 3000
                            });
                        }
                    }
    ...
    function editItem(id = null) {
        if (id) {
            ...
            $.ajax({
                ...
                dataType: 'json',
                success: function(response) {
                    $("#edit_company_name").val(response.company.company_name).focus();
                    $("#edit_contact_number").val(response.company.contact_number).focus();
                    $("#edit_alternate_contact_no").val(response.company.alternate_contact_no)
                        .focus(); // Added
                    $("#edit_email").val(response.company.email).focus(); // Added
                    $("#edit_person_name").val(response.company.person_name).focus(); // Added
                    $("#edit_designation").val(response.company.designation).focus(); // Added
                    $("#edit_person_number").val(response.company.person_number).focus(); // Added
                    var packageIds = JSON.parse(response.company.package_ids);
