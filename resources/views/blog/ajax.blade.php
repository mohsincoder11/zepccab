<script>
    ...
    $(document).ready(function() {
        ...
        $("#submitadd").on('click', function() {
            ...
            $(".text-danger").remove();
            $(".messages").html("");
            $("#addBlog").unbind('submit').bind('submit', function() {
                ...
                var postData = new FormData($("#addBlog")[0]);
                $.ajax({
                    cache:false,
                    contentType: false,
                    processData: false,
...
// remove the error
                        ...
                        if(response.success == true) {
                            ...
                            toastr.success('Added Successfully.', 'Blog', {timeOut: 5000});
                            $("#addBlog")[0].reset();
                ...
                $.ajax({
                    ...
                    success:function(response) {
                        if(response.success == true) {
                            ...
                            manageTable.ajax.reload(null, false);
                            // close the modal
                            toastr.success('Deleted Successfully.', 'Blog', {timeOut: 3000});
                            $("#modalConfirmDelete").modal('hide');
                            $(".loader-ajax").hide();

                        } else {
                            toastr.error('Not Deleted.', 'Blog', {timeOut: 3000});
                        }
                    }
                });
            }); // click remove btn
