<script>
    ...
    $(document).ready(function() {
        ...
        $("#submitadd").on('click', function() {
            ...
            $(".text-danger").remove();
            $(".messages").html("");
            $("#Addnotification").unbind('submit').bind('submit', function() {
                ...
                var postData = new FormData($("#Addnotification")[0]);
                $.ajax({
                    cache:false,
                    contentType: false,
                    processData: false,
...
// remove the error
                        ...
                        if(response.success == true) {
                            ...
                            toastr.success('Added Successfully.', 'Notification', {timeOut: 5000});
                ...
                $.ajax({
                    ...
                    success:function(response) {
                        if(response.success == true) {
                            ...
                            manageTable.ajax.reload(null, false);
                            // close the modal
                            toastr.success('Deleted Successfully.', 'Notification', {timeOut: 3000});
                            $("#modalConfirmDelete").modal('hide');
                            $(".loader-ajax").hide();

                        } else {
                            toastr.error('Not Deleted.', 'Notification', {timeOut: 3000});
                        }
                    }
                });
            }); // click remove btn
